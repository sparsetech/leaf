...
class WriterSpec extends AnyFunSuite {
  ...
  test("Generate table") {
    ...
    assert(
      nodes.head.asInstanceOf[pine.Tag[_]].toHtml ==
  ...
  test("Generate footnote") {
    val footnote = List(
      Node(NodeType.Paragraph, List(Node(NodeType.Footnote("first")))),
      Node(
  ...
  test("Generate todo box") {
    val todo =
      Node(NodeType.Todo, List(Node(NodeType.Text("Hello World"))))
  ...
  test("Generate section with ID") {
    ...
    assert(
      html.Writer.node(sectionWithId) ==
