...
class TagParserSpec extends AnyFunSuite {
  ...
  test("Parse tag without arguments") {
    assert(
      parse("<scala>") == Success(List(NodeType.OpenTag("scala", Map.empty)))
    )
  }

  test("Parse tag with one argument") {
    assert(
      parse("""<scala type="imports">""") == Success(
        List(NodeType.OpenTag("scala", Map("type" -> "imports")))
      )
    )
  }
  ...
  test("Parse tag with multiple arguments") {
    assert(
      parse("""<scala type="imports" name="test">""") == Success(
        List(
  ...
  test("Parse block extension") {
    assert(
      parse("""<tag key="value">""") == Success(
        List(NodeType.OpenTag("tag", Map("key" -> "value")))
      )
    )
    ...
    assert(
      parse("""<tag   key="val&quot;ue">""") == Success(
        List(NodeType.OpenTag("tag", Map("key" -> "val\"ue")))
      )
    )
    ...
    assert(
      parse("""<tag key="value" key2="value2">""") == Success(
        List(NodeType.OpenTag("tag", Map("key" -> "value", "key2" -> "value2")))
      )
    )
